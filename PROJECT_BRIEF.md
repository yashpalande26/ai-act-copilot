...
- Ingestion pipeline: fetch -> parse -> chunk with computed provenance -> embed -> store. [DONE — 1,255 provisions, 1,136 chunks, all embedded; 119/119 articles retrievable since ADR-28]
...
- Chunking → **1,136 chunks** with contextual prefixes (1,128 leaf paragraphs, points and annex points, plus the 8 single-paragraph article bodies restored by ADR-28) [DONE]
- Embeddings → all 1,136 embedded (1536-dim), resumable/idempotent [DONE]
...
- **ADR-23 follow-up: injection-guard false blocks.** Measured while gating ADR-24: the on-topic follow-up "Can it change the classification rules too?" was blocked by the gpt-4o-mini guard in 2 of 7 draws (0 of 41 items at ADR-23 time). A blocked on-topic question is a refused user. Options to measure: a deterministic allow rule for messages with no instruction verbs, or a guard prompt tuned on a false-block set; gate on 0 false blocks over repeats, not one run.
- **ADR-004 Step B, scoped (23 Sep 2026): the six annexes not yet ingested.** Each has a different layout in the source and none matches the grid-list parser: Annex I (List of Union harmonisation legislation): two `title-gr-seq-level-1` sections (A, B) with numbered bare paragraphs "2." to "21."; item 1 was deleted by M1 (a "▼M1 —————" marker with no text) and item 21 was inserted by M1, so the parser must carry section, number and amendment marker. Annex VII (conformity assessment based on QMS and technical documentation): five numbered sections, bare paragraphs with decimal sub-numbers ("3.1.") and grid lists beneath them. Annex VIII (registration information): three lettered sections (A, B, C), unnumbered bare paragraphs and 25 grid points. Annex X (large-scale IT systems): seven numbered sections each holding bare paragraphs naming Regulations. Annex XI (GPAI technical documentation): two sections whose titles span two title lines, bare paragraphs and 13 grid points. Annex XIV (notification codes): six HTML tables of `tbl-norm` cells with footnotes; a table parser, not a list parser. Suggested order: I first (the product-safety high-risk route, 20 list items, needs section plus marker handling), then VIII and XI (grid lists under sections, closest to the existing parser), then VII and X, and XIV last or as a table-to-text special case. Citation scheme to decide before building: `anx_I.sec_A.pt_2` or `anx_I.pt_2` with the section as heading; the structure map and `citation_label` must render whichever is chosen.
