...
- **ADR-13** — **ACCEPTED (21 Sep 2026): saved assessments.** New `assessment` table (migration `6aa3cd421b84`). `POST /assessments` takes **answers only**; the server re-derives the result, so a client cannot save what it did not earn. Owner-scoped reads, 404 no-leak. `engine_version = assess-1.<rules_hash>`, a canonical SHA-256 of the rule data (logic changes still need a hand bump of `ENGINE_MAJOR`). **Known limitation, not built:** reopening re-renders provision text live from the current corpus; not an immutable snapshot.
