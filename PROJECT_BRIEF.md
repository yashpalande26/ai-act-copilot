...
- **Run `alembic upgrade head` BEFORE deploying backend code, and set `APP_ENV=production` on the Railway service.** Order matters: `_write_trace_safe` swallows failures, so code writing the `environment` column against a table that lacks it fails silently, and every uncounted call is an open quota with no error anywhere. Smoke test: `/health` on Railway must read `"environment": "production"`.
