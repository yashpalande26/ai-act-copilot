...
- **ADR-24/25 follow-up: enable `CLARIFY_FOLLOWUP`.** After the verifier change the open items are (a) the understand step judging "we have an AI model in our company, is it a problem?" not a system description on some draws (2 of 4 today), which decides whether the follow-up fires at all: measure `understand_query` over repeats on the function-less descriptions and, if needed, name that case in its prompt with the on-topic set re-gated; (b) one triage draw in four withheld by gpt-4o; (c) the gold of the two Article 66(h) and 75(2) probes (valid paraphrase as authored, element change as gpt-4o reads them). Then re-run `AGENTIC_RAG=1 CLARIFY_FOLLOWUP=1 python evals/run_intent_eval.py --clarify` and `--only cl_03` three times; flip on 13 of 13 and 3 of 3.
