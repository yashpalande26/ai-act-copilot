...
- **ADR-24 follow-up: enable `CLARIFY_FOLLOWUP`.** Built and gated (12 of 13); the open gate is one underspecified item whose second pass retrieved and answered on the gold provision and was withheld by the gpt-4o-mini verifier. Decide: count that as grounded for this gate and flip the default, or fix the verifier's paraphrase over-withholding (ADR-20 b) first and re-run `AGENTIC_RAG=1 CLARIFY_FOLLOWUP=1 python evals/run_intent_eval.py --clarify`.
