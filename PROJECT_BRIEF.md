...
- **Multi-turn chat follow-ups (Part B).** Approach: a query-rewrite step (gpt-4o-mini) resolves a follow-up into a standalone query using the last N turns verbatim, then the existing RAG runs unchanged; the same last N turns are passed to generation for coherence. NOT rolling summarization: chats here are short. **Eval-gated:** requires a small follow-up eval set (turn sequences with gold answers) before it can be trusted, because a bad rewrite silently retrieves the wrong provision and nothing downstream notices. Cost per turn is negligible (sub-cent); the real cost is building the eval set. **Build trigger:** after first outreach, or when a real user asks for follow-ups.
