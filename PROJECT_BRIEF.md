...
- **Multi-turn chat follow-ups (Part B): BUILT, FLAG OFF (ADR-19, 22 Sep 2026).** Query rewrite on turn 2+ behind `FOLLOWUP_REWRITE`; passed the hard gates, missed the value gate (+12 vs +30 points of correct citation). Re-run `evals/run_followup_eval.py` after any retrieval change; enable only on a passing run. Remaining idea from the original note (pass the last N turns to generation) is not built.
