...
- **ADR-22 follow-up: clarifying trigger.** Decide whether the one clarifying question may fire on the generator's explain-mode abstention (measured 3/3 on the underspecified items) rather than the grader's; then re-gate `CLARIFY_FOLLOWUP` on the clarify set (grounding on the second pass, no provision asserted on law-silent items, no firing on off-topic, 0 leaks).
