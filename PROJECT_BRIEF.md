...
- **ADR-35** — **ACCEPTED (23 Sep 2026): the honest risk-tier answer for a described system; `RISK_TIER_FRAMING` default on.** The tiered explain instruction rewritten: provisions sorted by their own stated scope, the honest not-listed answer is a first-class response (none of the retrieved provisions names the use; any general obligation; the structured assessment confirms), adjacent provisions never mentioned, everyday-to-technical bridges name functions and never provisions. Safety line reframed: never cite a high-risk or prohibited provision for a use its scope does not cover, rather than cite nothing; the property-valuation gold expects the honest answer with Annex III forbidden. Gate: risk-tier absolute 0 over 42 draws, value 10/10 items at 3/3 (transparency 4/4 incl. the copilot, not-listed 4/4, property valuation, prohibited); on-topic 0 leaks, citation accuracy 0.884 to 0.953, recall 1.0. Known imperfection recorded in the ADR (card-fraud item quotes the transparency provision instead of the entry whose exception names fraud detection).
- **ADR-34** — **ACCEPTED (23 Sep 2026): the recital expansion runs only on explanation questions; `RECITAL_MAP` default on.** A deterministic detector (`recital_map.is_explanation_query`: "why", "reason for/behind", "rationale", "how come", "reasoning behind") gates the ADR-33 map; on the eval sets it fires on the four "why" items and nothing else. A direct lookup under the flag is ADR-32 exactly (recitals in the pool, demoted, trace string identical to flag off). Gate: every absolute gate 0, "why" items 4/4 grounded on recital plus provision, recall equal, relevance up, one deterministic difference on an item whose path the flag does not touch (flagged judgement in the ADR). Run file `adr34_recmap_intent_on_agentic_j2.json` is the on-topic baseline.
- **ADR-33** — **BUILT, superseded by ADR-34 (23 Sep 2026): a recital-to-provision map replaces the ADR-32 demotion behind `RECITAL_MAP`.** 157 edges in `provision_reference` (54 explicit from the recitals' own citations across 29 recitals, 103 semantic at one high-confidence link per recital; 132 of 180 recitals linked). With the flag on both retrieval legs exclude recitals and at most two linked recitals are placed behind the operative slice, only when their provision is served, re-chosen after the grader's cut. Full gate: every absolute gate 0, all four "why" items ground on recital plus operative provision, recall equal, faithfulness up, but citation accuracy on shared items 0.907 to 0.837 because two marginal direct items abstain under the longer context (measured systematic, cause is generator marginality, two fixes rejected). Resolved the same day by ADR-34's explanation-intent gate.
