...
- Ingestion pipeline: fetch -> parse -> chunk with computed provenance -> embed -> store. [DONE — 1,279 provisions, 1,156 chunks, all embedded; 119/119 articles retrievable since ADR-28; Annex I since ADR-29]
...
- Ingestion: fetch (content-hashed) → parse EUR-Lex HTML → **1,279 provisions** (119 articles + 9 annexes incl. Annex I with its two sections; 5 annexes still deferred per ADR-004 Step B) [DONE]
- Chunking → **1,156 chunks** with contextual prefixes (1,128 leaf paragraphs, points and annex points, 8 single-paragraph article bodies per ADR-28, 20 Annex I items per ADR-29) [DONE]
- Embeddings → all 1,156 embedded (1536-dim), resumable/idempotent [DONE]
